...
@property (nonatomic, strong) NSLayoutConstraint *imageWidthContraint;
...
#pragma mark - subview calculation 
...
- (void) layoutSubviews {
    ...
    CGSize usernameLabelSize = [self.usernameAndCaptionLabel sizeThatFits:maxSize];// asks for intrinsic size
    CGSize commentLabelSize = [self.commentLabel sizeThatFits:maxSize];//asks for instrinsic size
    
    //account for the possibility of missing data in our views since we now have the ability to delete
...
- (void) setMediaItem:(Media *)mediaItem {
    ...
    self.imageWidthContraint.constant = self.mediaItem.image.size.width / self.mediaItem.image.size.height * CGRectGetHeight(self.contentView.bounds);
    self.imageHeightConstraint.constant = self.mediaItem.image.size.height / self.mediaItem.image.size.width * CGRectGetWidth(self.contentView.bounds);
    }
    self.imageWidthContraint.constant = 100;
